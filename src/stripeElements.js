...
export function create(elementType, key_or_stripe, options = {}) {
  ...
  Stripe.createToken = tokenData =>
    Stripe.instance.createToken(element, tokenData);
  Stripe.createSource = sourceData =>
    Stripe.instance.createSource(element, sourceData);
  ...
  Stripe.redirectToCheckout = options =>
    Stripe.instance.redirectToCheckout(options);
  Stripe.retrievePaymentIntent = clientSecret =>
    Stripe.instance.retrievePaymentIntent(clientSecret);
  Stripe.handleCardPayment = (clientSecret, data) =>
    Stripe.instance.handleCardPayment(clientSecret, element, data);
  Stripe.handleCardSetup = (clientSecret, data) =>
    Stripe.instance.handleCardSetup(clientSecret, element, data);
  Stripe.handleCardAction = clientSecret =>
    Stripe.instance.handleCardAction(clientSecret);
  Stripe.confirmPaymentIntent = (clientSecret, data) =>
    Stripe.instance.confirmPaymentIntent(clientSecret, element, data);
  Stripe.createPaymentMethod = (cardType, data) =>
    Stripe.instance.createPaymentMethod(cardType, element, data);
  Stripe.clearElement = element.clear;
  return element;
}
